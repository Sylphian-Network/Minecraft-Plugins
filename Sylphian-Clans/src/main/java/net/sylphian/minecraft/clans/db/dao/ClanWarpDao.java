...
/**
 * JDBI DAO for the {@code sylphian_clan_warps} and {@code sylphian_clan_warp_access} tables.
 */
...
public interface ClanWarpDao {
     ...
     */
    @SqlUpdate("INSERT INTO sylphian_clan_warps (clan_id, name, world, x, y, z, yaw, pitch, icon, description, restricted) " +
    ...
    @SqlQuery("SELECT clan_id, name, world, x, y, z, yaw, pitch, icon, description, restricted " +
              "FROM sylphian_clan_warps WHERE clan_id = :clanId AND name = :name")
    ...
    @SqlUpdate("INSERT IGNORE INTO sylphian_clan_warp_access (clan_id, warp_name, player_uuid) " +
    ...
    @SqlUpdate("DELETE FROM sylphian_clan_warp_access WHERE clan_id = :clanId AND warp_name = :warpName AND player_uuid = :playerUuid")
    ...
    @SqlQuery("SELECT EXISTS(SELECT 1 FROM sylphian_clan_warp_access WHERE clan_id = :clanId AND warp_name = :warpName AND player_uuid = :playerUuid)")
    ...
    @SqlQuery("SELECT player_uuid FROM sylphian_clan_warp_access WHERE clan_id = :clanId AND warp_name = :warpName")
    ...
    @SqlQuery("SELECT warp_name FROM sylphian_clan_warp_access WHERE clan_id = :clanId AND player_uuid = :playerUuid")
    List<String> findAccessibleWarps(@Bind("clanId") String clanId, @Bind("playerUuid") String playerUuid);

    /** Raw row from the {@code sylphian_clan_warps} table. */
