...
/**
 * Parses the {@code nodes} section of Sylphian-Logging's config into engine
 * {@link NodeType}s. Node ids are prefixed with {@code "sylphian-logging:"}.
...
public final class NodeTypeConfigLoader {
    ...
    public List<NodeType> load(@Nullable ConfigurationSection nodes) {
        ...
        if (nodes == null) {
            logger.warning("No 'nodes' section in config.yml; no logging nodes will be registered.");
            return types;
        }
        ...
        logger.info("Logging node loading complete [" + types.size() + "] node type(s) registered.");
        return types;
    }
    ...
    private @Nullable NodeType parseNode(String key, ConfigurationSection node) {
        ...
        NodeInteraction interaction = parseEnum(NodeInteraction.class, node.getString("interaction"), NodeInteraction.BREAK, key, "interaction");
