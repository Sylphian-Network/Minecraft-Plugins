...
/**
 * Parses the {@code nodes} section of Sylphian-Mining's config into engine
 * {@link NodeType}s. Node ids are prefixed with {@code "sylphian-mining:"}.
...
public final class NodeTypeConfigLoader {
    ...
    public List<NodeType> load(@Nullable ConfigurationSection nodes) {
        ...
        if (nodes == null) {
            logger.warning("No 'nodes' section in config.yml; no mining nodes will be registered.");
            return types;
        }
        ...
        logger.info("Mining node loading complete [" + types.size() + "] node type(s) registered.");
        return types;
    }
