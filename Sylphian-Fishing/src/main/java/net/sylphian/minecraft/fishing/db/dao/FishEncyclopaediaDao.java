...
/**
 * Data Access Object for the sylphian_fishing_encyclopaedia table.
...
public interface FishEncyclopaediaDao {
    ...
    @SqlQuery("SELECT * FROM sylphian_fishing_encyclopaedia WHERE player_uuid = :uuid AND fish_id = :fishId")
    ...
    @SqlQuery("SELECT * FROM sylphian_fishing_encyclopaedia WHERE player_uuid = :uuid")
    List<FishEncyclopaediaRow> findAllForPlayer(@Bind("uuid") String uuid);

    /** Inserts a new catch record for a player. */
    @SqlUpdate("""
            INSERT INTO sylphian_fishing_encyclopaedia (player_uuid, fish_id, times_caught, biggest_weight, first_caught, last_caught)
    ...
    /** Represents a raw row from the sylphian_fishing_encyclopaedia table. */
    record FishEncyclopaediaRow(String playerUuid, String fishId, int timesCaught, double biggestWeight, long firstCaught, long lastCaught) {}
}